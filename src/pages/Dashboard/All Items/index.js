...
import { Table, Spin, Image, Button, Modal, Form, Input } from "antd";
...
const All = () => {
  ...
  const handleOk = async () => {
    try {
      ...
      await updateDoc(doc(firestore, "items", editingItem.id), values);
      setItems(items.map(item => (item.id === editingItem.id ? { ...item, ...values } : item)));
